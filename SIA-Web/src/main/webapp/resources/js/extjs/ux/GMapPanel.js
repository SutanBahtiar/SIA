...
Ext.define('Ext.ux.GMapPanel', {
    ...
    onMapReady : function(){
        ...
        this.addPolyline(this.polylines);
    ...
    myTest:function(obj){
        ...
        tempLatLang = obj.getPosition();
        console.info(tempLatLang);
    },

    //center location
    showPotensiGas:function(obj){

        var maps = this.getMap();

        this.getMap().setZoom(10);
        this.getMap().setCenter(obj.getPosition());

        var neighborhoods = [
            new google.maps.LatLng(-6.43267, 106.04691),
            new google.maps.LatLng(-6.35079, 106.23917),
            new google.maps.LatLng(-6.21838, 106.04004)
        ];
        ...
        for (var i = 0; i < neighborhoods.length; i++) {
            setTimeout(function() {
                s_markers.push(new google.maps.Marker({
                    position: neighborhoods[s_iterator],
                    map: maps,
                    draggable: false,
    ...
    dropSumberGas :function(obj,latVal, langVal){

        var v_latlang = new google.maps.LatLng(latVal, langVal);
        obj.setPosition(v_latlang);

        if(tempLatLang != null){
            console.log('---- not null ----> ' + obj.status);
