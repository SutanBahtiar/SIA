Ext.define('Ifuel.controller.Role', {
    extend: 'Ext.app.Controller',

    views: [ 'role.List', 'role.Edit'  ],
    allowWrite: Ifuel.util.SecurityHelper.isWriteAllowed("ROLE_WRITE"),
    models: [ 'Role'],
    stores: [ 'Roles'],
    ...
    deleteRole: function (button) {
        ...
        if (record) {
            Ext.Ajax.request({
                url: Ifuel.config.roleUrl + record.data.id,
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json'
                },
                success: function (response, options) {
                    Ifuel.util.MessageBox.showMessageBoxDelete(response, options, 'Role');
                    self._doGridRefresh();
                },
    ...
    updateRole: function (button) {
        ...
        if (form.getForm().isValid()) {
            Ext.Ajax.request({
                ...
                success: function (response, options) {
                    Ifuel.util.MessageBox.showMessageBoxPostPut(response, options, 'Role');
                    self._doGridRefresh();
                },
                failure: function (response, options) {
                    Ifuel.util.MessageBox.showMessageBoxPostPut(response, options, 'Role');
