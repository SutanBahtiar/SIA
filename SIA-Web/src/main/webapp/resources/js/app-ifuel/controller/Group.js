Ext.define('Ifuel.controller.Group', {
    extend: 'Ext.app.Controller',
    views: [ 'group.List', 'group.Edit' ],
    allowWrite: Ifuel.util.SecurityHelper.isWriteAllowed("GROUP_WRITE"),
    models: [ 'Group'],
    stores: [ 'Groups'],
    refs: [
        {
    ...
    deleteGroup: function (button) {
        ...
        if (record) {
            Ext.Ajax.request({
                url: Ifuel.config.groupUrl + record.data.id,
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json'
                },
                success: function (response, options) {
                    Ifuel.util.MessageBox.showMessageBoxDelete(response, options, 'Group');
                    self._doGridRefresh();
                },
        ...
        if (form.getForm().isValid()) {
            if (record) {
                Ext.Ajax.request({
                    ...
                    success: function (response, options) {
                        Ifuel.util.MessageBox.showMessageBoxPostPut(response, options, 'Group');
                        self._doGridRefresh();
                    },
                    failure: function (response, options) {
                        Ifuel.util.MessageBox.showMessageBoxPostPut(response, options, 'Group');
