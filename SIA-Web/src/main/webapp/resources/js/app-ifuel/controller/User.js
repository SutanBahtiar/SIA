Ext.define('Ifuel.controller.User', {
    extend: 'Ext.app.Controller',
    views: [ 'user.List', 'user.Edit' ],
    allowWrite: Ifuel.util.SecurityHelper.isWriteAllowed("USER_WRITE"),
    models: [ 'User'],
    stores: [ 'Users'],
    refs: [
        {
    ...
    deleteUser: function (button) {
        ...
        if (record) {
            Ext.Ajax.request({
                url: Ifuel.config.userUrl + record.data.id,
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json'
                },
                success: function (response, options) {
                    Ifuel.util.MessageBox.showMessageBoxDelete(response, options, 'User');
                    self._doGridRefresh();
                },
        ...
        if (form.getForm().isValid()) {
            if (record) {
                Ext.Ajax.request({
                    ...
                    success: function (response, options) {
                        Ifuel.util.MessageBox.showMessageBoxPostPut(response, options, 'User');
                        self._doGridRefresh();
                    },
                    failure: function (response, options) {
                        Ifuel.util.MessageBox.showMessageBoxPostPut(response, options, 'User');
